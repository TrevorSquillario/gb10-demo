        ...
        while not stop_event.is_set():
            ...
            for det in detections:
                ...
                if track_id >= 0:
                    ...
                    # Store latest JPEG crop for the UI panel
                    h_im, w_im = im.shape[:2]
                    cx1, cy1 = max(0, x1), max(0, y1)
                    cx2, cy2 = min(w_im, x2), min(h_im, y2)
                    if (cx2 - cx1) > 10 and (cy2 - cy1) > 10:
                        crop_bgr = im[cy1:cy2, cx1:cx2]
                        ok, buf = cv2.imencode(
                            ".jpg", crop_bgr, [cv2.IMWRITE_JPEG_QUALITY, 80]
                        )
                        if ok:
                            with ctx.crops_lock:
            ...
            if now - ctx.last_deepface_time >= ctx.ANALYSIS_INTERVAL and snapshot_detections:
                ctx.deepface_service.submit(im, snapshot_detections)
