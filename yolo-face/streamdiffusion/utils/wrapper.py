...
class StreamDiffusionWrapper:
    ...
    def postprocess_image(
        ...
        """
        # sanitize NaN/Inf values that may appear during fp16 operations
        image_tensor = image_tensor.cpu()
        try:
            image_tensor = torch.nan_to_num(image_tensor, nan=0.0, posinf=1.0, neginf=-1.0)
        except Exception:
            # fallback: replace via numpy if torch.nan_to_num unavailable for dtype
            img_np = image_tensor.numpy()
            img_np = np.nan_to_num(img_np, nan=0.0, posinf=1.0, neginf=-1.0)
        ...
        try:
            ...
            if acceleration == "tensorrt":
                ...
                if not os.path.exists(unet_path):
                    ...
                    # For SDXL the UNet receives the concat of both CLIP hidden
                    # states (768 + 1280 = 2048) and also needs added_cond_kwargs
                    # {text_embeds (1280), time_ids (6)} traced as positional inputs.
                    if self.sdxl and hasattr(stream.pipe, "text_encoder_2"):
                        _embed_dim = (
                            stream.text_encoder.config.hidden_size
                            + stream.pipe.text_encoder_2.config.hidden_size
                        )
                        _pooled_dim = stream.pipe.text_encoder_2.config.projection_dim
                    else:
                        _embed_dim = stream.text_encoder.config.hidden_size
                        _pooled_dim = 1280
                    unet_model = UNet(
                        fp16=True,
                        ...
                        is_sdxl=self.sdxl,
                        pooled_embedding_dim=_pooled_dim,
                    )
                    compile_unet(
                        stream.unet,
                        unet_model,
                ...
                # Preserve unet metadata needed by pipeline.prepare() / _get_add_time_ids()
                unet_config = stream.unet.config
                unet_add_embedding = stream.unet.add_embedding
