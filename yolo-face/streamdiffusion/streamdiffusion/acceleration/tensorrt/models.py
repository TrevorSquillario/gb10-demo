...
class UNetWrapper(torch.nn.Module):
    """Wraps the SDXL UNet so torch.onnx.export can trace added_cond_kwargs
    as flat positional inputs instead of a dict (which the tracer cannot handle)."""
...
class UNet(BaseModel):
    ...
    def get_shape_dict(self, batch_size, image_height, image_width):
        ...
        if self.is_sdxl:
            shape_dict["text_embeds"] = (2 * batch_size, self.pooled_embedding_dim)
    ...
    def get_sample_input(self, batch_size, image_height, image_width):
        ...
        if self.is_sdxl:
            inputs += (
                ...
                torch.zeros(2 * batch_size, 6, dtype=dtype, device=self.device),
            )
        return inputs

    def wrap_network(self, network: torch.nn.Module) -> torch.nn.Module:
        """Return a UNetWrapper for SDXL (flattens added_cond_kwargs for ONNX tracing),
        or the original network unchanged for SD 1.x."""
        if self.is_sdxl:
            return UNetWrapper(network)
        return network
