...
class Engine:
    ...
    def allocate_buffers(self, shape_dict=None, device="cuda"):
        # TensorRT 10+ uses a named-tensor API; the old binding-indexed API was removed.
...
def export_onnx(
    ...
    with torch.inference_mode(), torch.autocast("cuda"):
        ...
        # For SDXL UNets, added_cond_kwargs must arrive as named positional tensors
        # because torch.onnx.export cannot trace through dict kwargs. model_data
        # provides wrap_network() which inserts a thin adapter layer.
        if hasattr(model_data, "wrap_network"):
            model = model_data.wrap_network(model)
        torch.onnx.export(
            model,
            inputs,
            onnx_path,
